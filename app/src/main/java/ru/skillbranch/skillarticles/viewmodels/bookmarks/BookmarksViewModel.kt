...
class BookmarksViewModel(handle: SavedStateHandle) :
    BaseViewModel<BookmarkArticlesState>(handle, BookmarkArticlesState()) {
    ...
    private val listData = Transformations.switchMap(state) {
        when {
            it.isSearch && !it.searchQuery.isNullOrBlank() ->
                buildPagedList(repository.searchBookmarkArticles(it.searchQuery))
    ...
    private fun buildPagedList(
        ...
        // if all articles
        if (dataFactory.strategy is ArticleStrategy.AllArticle) {
    ...
    private fun itemAtEndHandle(lastLoadArticle: ArticleItemData) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                notify(
                    Notify.TextMessage("Load from network articles " +
...
data class BookmarkArticlesState(
